/**
 * Bulk import matched client .tud (+ companion .xlsx) from the Mahrab pattern
 * archive into live pattern_library + erp-pattern-files.
 *
 * Reuses the Base Patterns /ALL importer logic. Idempotent by filename.
 * Does not invent clients for ambiguous folders — only ensures previously
 * approved CREATE_CLIENT_FOLDERS exist.
 *
 * Usage:
 *   node --experimental-strip-types scripts/import-mahrab-pattern.mjs
 *   node --experimental-strip-types scripts/import-mahrab-pattern.mjs --root "/Users/ralphrahme/Downloads/Mahrab pattern"
 *   node --experimental-strip-types scripts/import-mahrab-pattern.mjs --dry-run
 *   node --experimental-strip-types scripts/import-mahrab-pattern.mjs --limit 5
 *   node --experimental-strip-types scripts/import-mahrab-pattern.mjs --folders "Ralph Rahme|Ibrahim Al Shwemi"
 */
...
/** House / brand / calibration reference packs — skip (do not invent clients). */
...
/**
 * Manual ERP matches (folder → client code).
 * Ajlan family kept DISTINCT — do not collapse Abdel Aziz* into Ajlan Mohamad.
 */
const FORCE_MATCH = {
  // Ajlan cluster (careful)
  ...
  "Mark": "FR-0226-0016",
  // Previously created from ALL import
...
/**
 * Only ensure these exist if missing (already approved). Do not invent others.
 */
...
/** Avoid wrong fuzzy matches (Sami ≠ Samer Al Jameel). */
...
async function ensureCreatedClients(admin) {
  ...
  for (const folder of CREATE_CLIENT_FOLDERS) {
    ...
    const client = {
      ...
      notes: `Created from Mahrab pattern/${folder} import.${names.mononym ? " Mononym folder — confirm legal last name." : ""}`,
      is_active: true,
      client_kind: "person",
    };
    clients.push(client);
...
function detectFabric(...texts) {
  ...
  // Fabric article codes often appear as BOL.009, N781050, 722042, etc.
  const code =
...
async function main() {
  ...
  for (const name of top) {
    ...
    if (folderFilter && !folderFilter.has(name)) continue;
    clientFolders.push(name);
  }
  console.log(`Classification: clientCandidates=${clientFolders.length} reference=${skippedReference.length} junk=${skippedJunk.length}`);
  if (skippedReference.length) console.log(`Skipped reference: ${skippedReference.join(", ")}`);
  if (skippedJunk.length) console.log(`Skipped junk: ${skippedJunk.join(", ")}`);
  ...
  for (const folder of clientFolders) {
    ...
    matchedFolders.push({ folder, code: match.client.code, score: match.score, forced: match.forced });
  ...
  console.log(
    `Upload plan: ${plan.length} client garment patterns from ${new Set(plan.map((p) => p.folder)).size} matched folders (of ${matchedFolders.length} matched client folders)`
  );
  for (const m of matchedFolders) {
    console.log(`  MATCH ${m.folder} → ${m.code} (score=${m.score.toFixed(2)}${m.forced ? " forced" : ""})`);
  }
    ...
    try {
      ...
      if (!pattern) {
        ...
        pattern = {
          ...
          description: `${row.garment} pattern imported from Mahrab pattern/${row.folder}`,
  ...
  const summaryPath = "/tmp/mahrab-pattern-import-log.json";
  writeFileSync(
    summaryPath,
    JSON.stringify(
      {
        ...
        clients_touched: [...new Set(log.uploaded.map((u) => u.label.split(" ")[0]))],
      },
      null,
      2
    )
  ...
  console.log("\nDone.");
  console.log(`  matched client folders: ${matchedFolders.length}`);
  console.log(`  unmatched client folders: ${unmatchedFolders.length}`);
  console.log(`  skipped reference: ${skippedReference.length}`);
  console.log(`  skipped junk: ${skippedJunk.length}`);
  console.log(`  created patterns: ${log.created_patterns.length}`);
  console.log(`  uploaded: ${log.uploaded.length}`);
  console.log(`  skipped (idempotent): ${log.skipped.length}`);
