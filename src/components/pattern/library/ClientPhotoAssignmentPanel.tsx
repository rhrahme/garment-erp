...
import { Camera, Link2, Unlink } from "lucide-react";
import type {
  ClientFabricBoard,
...
export function ClientPhotoAssignmentPanel({
  ...
  const fabricOptions = useMemo(() => {
    const linked = new Set(linkedLineIds);
    const preferred = fabricRows.filter(
      (row) => linked.has(row.line_id) || row.assigned_pattern?.pattern_id === patternId
    );
    const rest = fabricRows.filter((row) => !preferred.some((p) => p.line_id === row.line_id));
    return [...preferred, ...rest];
  }, [fabricRows, linkedLineIds, patternId]);
              ...
              >
                <button
                  ...
                  onClick={() => {
                    ...
                    setAssignLineId(photo.assigned_fabric_line_id ?? "");
                  }}
                ...
                <div className="space-y-1.5 p-2.5">
                  ...
                  {photo.assigned_client_pattern_id === patternId ? (
                    <p className="text-[10px] text-slate-400">Linked to this pattern</p>
                  ) : null}
                </div>
              </div>
            );
      ...
      {selectedPhoto ? (
        <div className="mt-3 rounded-lg border border-indigo-100 bg-indigo-50/50 p-3">
          <p className="mb-2 text-xs font-semibold text-slate-700">
            Assign photo to fabric / article
          </p>
          <div className="flex flex-wrap items-end gap-2">
            <label className="min-w-[16rem] flex-1 text-xs">
              <span className="mb-1 block text-slate-600">Fabric line</span>
              <select
                value={assignLineId}
            ...
            >
              {busyId === selectedPhoto.id ? "Saving..." : "Save assignment"}
            </button>
            <button
              type="button"
              onClick={() => {
