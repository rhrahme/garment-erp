...
export function LineReductionSuggestionsPanel({
  ...
  saving?: boolean;
  /** When false, the control is hidden (e.g. non-draft invoices). */
  enabled?: boolean;
}) {
  const [scanned, setScanned] = useState(false);
  const suggestions = useMemo(
    () => (scanned ? detectInvoiceLineReductions(lines) : []),
    [lines, scanned]
  );
  ...
  return (
    <div className="space-y-3">
      ...
      {enabled && scanned && suggestions.length === 0 && (
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-5 py-4">
          <p className="text-sm text-slate-700">
            No combinations found — no split suit pieces, duplicate fabric numbers, or same-price lines to merge.
          </p>
        </div>
      )}
