...
/** Dev only — open in browser to sign in as QC without a password. */
export async function GET(request: Request) {
  ...
  if (!email || !isClientManagerEmail(email) || !resolveDevImpersonationEmail(email)) {
    return NextResponse.json({ error: "Email not allowed." }, { status: 400 });
  }

  const redirect = NextResponse.redirect(new URL(defaultPathForSession(true), request.url));
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!email || !isClientManagerEmail(email)) {
      return NextResponse.json({ error: "Only client manager emails can be impersonated in dev." }, { status: 400 });
    }
