...
/** Backfill null garment_type / fabric_number from the live SO sticker lookup. */
export function enrichSewingSessionGarmentFields(session: SewingSession): SewingSession {
  ...
  if (!needsType && !needsFabric) return session;
...
export function enrichSewingSessionsGarmentFields(sessions: SewingSession[]): SewingSession[] {
  ...
  return sessions.map((session) => {
    const key = `${session.id}|${session.scan_code}|${session.production_code}|${session.garment_type ?? ""}`;
