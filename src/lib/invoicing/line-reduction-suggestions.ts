...
/**
 * A merged line prints one price, so the fabric cost belongs in the key too.
 * The mill is no longer optional here - `buildConsolidationMergeKey` always
 * keys on `fabric_brand`.
 */
function reductionOptions(options?: LineReductionOptions): LineReductionOptions {
  return { includeFabricCost: true, ...options };
}
