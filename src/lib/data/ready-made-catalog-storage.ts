...
/** Ready-made style photos share the client photos bucket, own subdir. */
export const READY_MADE_CATALOG_SUBDIR = "ready-made-catalog";
...
function objectPath(filename: string): string {
  return `${READY_MADE_CATALOG_SUBDIR}/${filename}`;
}

export async function writeReadyMadeCatalogImage(
  filename: string,
  content: Buffer,
  contentType: string
): Promise<void> {
...
export async function readReadyMadeCatalogImage(filename: string): Promise<Buffer | null> {
  if (isSupabaseDocumentsStorage()) {
    const admin = getSupabaseAdmin();
...
export async function deleteReadyMadeCatalogImage(filename: string): Promise<void> {
  if (isSupabaseDocumentsStorage()) {
    const admin = getSupabaseAdmin();
    if (!admin) return;
