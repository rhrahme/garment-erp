...
export const ERP_DOCUMENT_DEFINITIONS: ErpDocumentDefinition[] = [
  ...
  {
    key: "ready_made_catalog",
    label: "Ready-made catalog photos",
    description: "Style and size photos for ready-made garments",
    category: "production",
    appHref: "/ready-made",
    sourcePath: "src/data/ready-made-catalog.json",
  },
  {
    key: "entity_images",
    label: "Fabric and garment photos",
    description: "User uploads on fabrics, garment types, and sales-order articles",
    category: "production",
    sourcePath: "src/data/entity-images.json",
  },
  {
...
export function recordSummaryForKey(key: ErpDocumentKey, data: unknown): string {
  ...
  switch (key) {
    ...
    case "ready_made_catalog":
      return `${Array.isArray(record.garments) ? record.garments.length : 0} garments`;
    case "entity_images":
      return `${Array.isArray(record.albums) ? record.albums.length : 0} albums`;
