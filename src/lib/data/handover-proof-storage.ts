...
/** Delivery / handover proof photos. Separate from receive-sample images. */
export const HANDOVER_PROOF_SUBDIR = "handover-proof";
...
function objectPath(filename: string): string {
  return `${HANDOVER_PROOF_SUBDIR}/${filename}`;
}

export async function writeHandoverProofImage(
  filename: string,
  content: Buffer,
  contentType: string
): Promise<void> {
...
export async function readHandoverProofImage(filename: string): Promise<Buffer | null> {
  if (isSupabaseDocumentsStorage()) {
    const admin = getSupabaseAdmin();
...
export async function deleteHandoverProofImage(filename: string): Promise<void> {
  if (isSupabaseDocumentsStorage()) {
    const admin = getSupabaseAdmin();
    if (!admin) return;
